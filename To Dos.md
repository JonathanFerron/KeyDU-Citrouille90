...
1. Extend consumer/system keycodes — add the following missing codes to keycode.h and
...
2. Replace MACRO_ACTION_WAIT busy-wait in macro.c with kbd_stage_wait()
...
3. Test LED out. Wire external LEDs with resistors on the CNano breadboard (CNano's on-board
   LED on PF2 is a matrix column — use separate LEDs on the appropriate TCA0 WO pins instead).

4. Sleep mode — enable AVR IDLE sleep in the main loop. The structure is already staged:
...
5. Stale Report on USB Resume (Phantom Keypresses)
...
7. Time TCB0_INT_vect to 200us prior to expected SOF arrival (800 us after prior SOF arrival)

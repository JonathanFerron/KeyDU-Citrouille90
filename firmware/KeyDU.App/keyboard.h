/* keyboard.h — Citrouille90 / AVR64DU32
   ...
   Keyboard report:
     ...
     kbd_stage()       — enqueue current report into the kbd report queue.
                         Non-blocking.  See usb_hid.h for queue design.
   ...
   Both helpers call kbd_stage() twice.  The queue makes each staged report
   visible to hid_flush() independently — no SOF race.
...
/* Enqueue the current keyboard report into the USB HID report queue.
...
/* Send a modifier + key combo: press, stage, release, stage.
   ...
   implementation and queue-correct staging applies to all callers. */
